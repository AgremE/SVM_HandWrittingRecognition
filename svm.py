...
class SupportVectorMachine(classificationMethod.ClassificationMethod):
    ...
    def train(self, trainingData, trainingLabels, validationData, validationLabels):
        ...
        for c1 in range(len(self.legalLabels)):
            for c2 in range(c1+1, len(self.legalLabels)):
                ...
                X2 = np.asarray([x for i,x in enumerate(trainingData) if trainingLabels[i] == c2]) # Data with label c2
                X = np.vstack((X1, X2))
    ...
    def trainSVM(self, X, t, C, kernel):
        ...
        N, D = X.shape # D = 13 and N  = 27 for the case of 100
        G_1 = np.identity(N)
        G_2 = -1*G_1
        G = np.vstack((G_1,G_2))
        A = t[:,None].T
        ...
        for i in range(N):
            h[i+N] = 0
        l_multipliers = self.quadraticProgrammingSolver(P, q, G, h, A, b)
    ...
    def predictSVM(self, x, supportMultipliers, supportVectors, supportVectorLabels, bias, kernel):
        ...
        for i in range(len(supportVectors)):
            wTran_x += np.dot(supportMultipliers[i],np.dot(supportVectorLabels[i],kernel(x,supportVectors[i])))
        y_x = wTran_x + bias
        if y_x > 0:
            result = 1
        elif y_x < 0:
